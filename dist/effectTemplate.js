...
export class EffectTemplate {
    ...
    apply(target, level = 1, expire = Infinity, expireOffset = -1) {
        ...
        let effect = new Effect(this.name, this.id, this.descriptionCallback(level), modifiers, level);
...
export const StrengthEffectTemplate = new EffectTemplate("力量", "strength", (level) => `攻击力提升${Math.round(5 + level * 10)}%`, (target, level, expire) => {
    return [
        target.attackDamage
            .area(1)
...
export const WeaknessEffectTemplate = new EffectTemplate("虚弱", "weakness", (level) => `攻击力降低${Math.round(10 + level * 10)}%`, (target, level, expire) => {
    return [
        target.attackDamage
            .area(1)
