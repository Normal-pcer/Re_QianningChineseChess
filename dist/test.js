...
class TypeRegistry {
    ...
    static revive(obj) {
        ...
        let idToRevived = new Map();
        const revive_recursion = (obj) => {
            ...
            if (obj?.__registeredType) {
                ...
                if (Ctor) {
                    ...
                    if (typeof Ctor.fromJSON === "function") {
                        const result = Ctor.fromJSON(obj);
                        if (result instanceof Serializable) {
                            if (obj?.__refId !== undefined)
                                idToRevived.set(obj.__refId, result);
                            return result;
                        }
                        else {
                            throw new SerializeError(`Result of '${obj.__registeredType}'.fromJSON not extends Serializable.`);
                        }
                    }
                    // 实例方法
                    if (typeof Ctor.prototype.fromJSON === "function") {
                        ...
                        if (result instanceof Serializable) {
                            ...
                            throw new SerializeError(`Result of '${obj.__registeredType}'.prototype.fromJSON not extends Serializable.`);
                        }
                    }
                    let instance;
                    if (Ctor.prototype.getConstructArgs) {
                        ...
                        try {
                            ...
                            throw new SerializeError(`Cannot create instance of type '${obj.__registeredType}' with default constructor.`);
                        }
                        // 直接赋值所有属性
                    ...
                    if (instance instanceof Serializable) {
                        if (obj?.__refId !== undefined)
                            idToRevived.set(obj.__refId, instance);
                        return instance;
                    }
                    else {
                        throw new SerializeError(`'${obj.__registeredType}' not extends Serializable.`);
                    }
                }
                else {
                    console.log("Error on reviving object: ", obj);
                    throw new SerializeError(`Cannot find type '${obj.__registeredType}' in registry.`);
                }
            }
            else {
                let result = {};
        ...
        const restore_ref_recursion = (obj) => {
            if (typeof obj !== "object" || obj === null) {
                return;
            }
            if (Array.isArray(obj)) {
                for (let i = 0; i != obj.length; i++) {
                    if (obj[i]?.__toRefId !== undefined) {
                        if (!idToRevived.has(obj[i].__toRefId)) {
                            ...
                            throw new SerializeError(`Cannot find object with refId ${obj[i].__toRefId}`);
                        }
                ...
                for (let key in obj) {
                    if (obj[key]?.__toRefId !== undefined) {
                        if (!idToRevived.has(obj[key].__toRefId)) {
                            console.log("Error on restoring ref: ", obj);
                            throw new SerializeError(`Cannot find object with refId ${obj[key].__toRefId}`);
                        }
...
class Serializable {
     ...
     */
    static RESERVED_KEYS = ["__registeredType"];
    /**
    ...
    serialize() {
        ...
        let need_record_id = new Set(); // 需要记录在结果中的所有 __refId
        const serialize_recursion = (obj) => {
            // 如果是平凡类型
            if (typeof obj !== "object" || obj === null) {
                return obj;
            }
            ...
            if (obj instanceof Serializable) {
                const typeName = obj.__registeredType;
                ...
                // 筛选键名
                const keys = obj.getSerializeKeys?.() ??
                    Object.keys(obj).filter((key) => !Serializable.RESERVED_KEYS.includes(key) &&
                        Object.prototype.hasOwnProperty.call(obj, key));
                let result = { __registeredType: typeName };
...
function testArrayCircular() {
    try {
        const arr = [];
        arr.push(arr); // 创建数组自引用
        const temp = new (class extends Serializable {
            data = arr;
        });
        temp.serialize();
        console.error("数组循环引用测试未抛出错误");
    }
    catch (e) {
        if (e instanceof SerializeError)
            console.assert(e.message.includes("Cannot serialize array with circular reference"), "应检测到数组循环引用错误");
    }
}
testArrayCircular();
