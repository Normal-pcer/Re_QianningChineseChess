...
export const defaultQuasiMoveTargets: { [key in DamageType]: (piece: Piece, target: Piece) => Position } =
    {
...
export const defaultRepelTargets: { [key in DamageType]: (piece: Piece, target: Piece) => Position } = {
    [DamageType.None]: (piece, target) => {
        return piece.position;
    },
    [DamageType.MeleeLow]: (piece, target) => {
